...
namespace rubo {
    ...
    void confugure(int clock, int latch, int data) {
        ...
        left_timer_id = timer.setInterval(TURN_PERIOD, left_turn_blink);
        right_timer_id = timer.setInterval(TURN_PERIOD, right_turn_blink);
        alarm_timer_id = timer.setInterval(TURN_PERIOD, alarm_blink);
